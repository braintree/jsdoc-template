...
function highlightActiveHash(event) {
  ...
  if(event && event.originalEvent.oldURL) {
    ...
    if((oldUrl.indexOf("#")>-1)) {
      var oldSubSectionElement = $("#" + getCurrentSectionName() + "-" + oldUrl.substring(oldUrl.indexOf("#")+1) + "-nav");
...
function getCurrentSectionName() {
  ...
  // remove the word module- if its in the url
...
function getCurrentHashName() {
  ...
  if(pageSubSectionHash) {
    var pageSubSectionId = pageSubSectionHash.substring(1).replace('.','');
